...
export const minificationDeferController = async (req, res, next) => {
  try {
    ...
    while (hasNextPage) {
      const getAllThemeFilesResponse = await queryDataWithVariables(
        res,
        GetAllThemeFiles,
        {
          count: PER_PAGE,
      ...
      const themeFiles =
        getAllThemeFilesResponse.data.themes.edges[0].node.files;
    ...
    const liquidFiles = allThemeFiles.filter(
      (file) =>
    ...
    for (const file of liquidFiles) {
      ...
      const scriptRegex =
        /<script([^>](?!defer|async|type=['"]application\/json['"]|type=['"]application\/ld\+json['"]|type=['"]text\/template['"]))*>.*?<\/script>/gs;

      const scriptTags = content.match(scriptRegex) || [];

      for (const originalScript of scriptTags) {
        if (
          originalScript.includes(" defer") ||
          originalScript.includes(" async")
        ) {
          continue;
        }

        if (
          originalScript.includes("document.write") ||
        ...
        if (
          originalScript.includes("analytics") ||
...
export const optimizedLoadingController = async (req, res, next) => {
  try {
    ...
    // });
    console.log(
      "🚀 ~ optimizedLoadingController ~ allThemeFiles:",
      allThemeFiles
    );
    return res.status(200).json({
      success: true,
