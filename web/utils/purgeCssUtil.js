...
/**
 * Runs PurgeCSS on the provided Liquid, JS, and CSS files to remove unused CSS.
 *
 * @async
 * @function getOptimizedCss
 * @param {Array<{content: string, filename: string}>} liquidFiles - Array of Liquid file objects with `content` and `filename`.
 * @param {Array<{content: string, filename: string}>} jsFiles - Array of JavaScript file objects with `content` and `filename`.
 * @param {Array<{content: string, filename: string}>} cssFiles - Array of CSS file objects with `content` and `filename`.
 * @returns {Promise<Array>} The result from PurgeCSS, containing optimized CSS for each file.
 *
 * @example
 * const result = await getOptimizedCss(liquidFiles, jsFiles, cssFiles);
 * // result is an array of PurgeCSS output objects
 */
